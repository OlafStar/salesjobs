...
const CreateJobPostingForm = () => {
  ...
  const {setOfferDetails} = useCreateOfferContext();
  ...
  function onSubmit(values: z.infer<typeof createOfferScheme>) {
    ...
    router.push('/offer-create/preview');
  }

  //TODO: Delete this before prod
  useEffect(() => {
    // Set form values from mock data
    Object.entries(createOfferMockData).forEach(([key, value]) => {
      //@ts-ignore
      form.setValue(key as keyof typeof mockData, value);
    });
  }, [form]);
          ...
          <div className="flex gap-8">
            <CreateOfferUpload
              onComplete={async (file) => {
                form.setValue('company.logo', file[0]);
              }}
            />
                ...
                <FormItem className="w-48">
                  ...
                  <FormControl>
                    ...
                    >
                      ...
                      <SelectContent>
                        {createOfferScheme.shape.experience.options.map((exp) => (
          ...
          <FormField
            ...
            render={({field, fieldState: {error}}) => (
              <FormItem>
                ...
                >
                  {createOfferScheme.shape.operatingMode.options.map((mode) => (
          ...
          <FormField
            ...
            render={({field, fieldState: {error}}) => (
              <FormItem>
                ...
                >
                  {createOfferScheme.shape.typeOfWork.options.map((mode) => (
                    <div key={mode} className="flex items-center space-x-2">
                      <RadioGroupItem
                        value={mode}
                        id={mode}
                        onChange={(val) => field.onChange(val.currentTarget.value)}
                      />
                      <Label htmlFor={mode}>{mode}</Label>
                    </div>
                  ))}
